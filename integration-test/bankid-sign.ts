...
if (!BANKID_SIGN_URL) throw new Error('BANKID_SIGN_URL is not set');
if (!NATIONAL_IDENTITY_NUMBER) throw new Error('NATIONAL_IDENTITY_NUMBER is not set');
if (!BANKID_TEST_OTP) throw new Error('BANKID_TEST_OTP is not set');
if (!BANKID_TEST_PASSWORD) throw new Error('BANKID_TEST_PASSWORD is not set');
...
try {
  ...
  // Page navigates to idp.preprod.esign-stoetest.cloud — BankID UI is in Norwegian
