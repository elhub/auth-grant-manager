...
class CreateRequestHandler(private val repo: RequestRepository) {
    suspend operator fun invoke(command: CreateRequestCommand): Either<AuthorizationRequestProblem, AuthorizationRequest> =
        repo.create(command.type, command.requester, command.requestee)
}
