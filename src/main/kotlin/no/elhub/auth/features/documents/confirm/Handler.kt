...
class Handler(
    ...
    suspend operator fun invoke(command: Command): Either<ConfirmError, Unit> =
        either {
            ...
            val signatoryIdentifier =
                signatureService
                    .validateSignaturesAndReturnSignatory(command.signedFile, document.file)
                    .mapLeft {
                        log.error("Validate signature error occurred: {}", it)
                        ConfirmError.ValidateSignaturesError(it)
                    }.bind()

            val actualSignatoryParty =
                partyService
                    .resolve(signatoryIdentifier)
            ...
            transaction {
                    ...
                    documentRepository
                        .confirm(
                            ...
                            when (error) {
                                ...
                                is RepositoryWriteError.UnexpectedError,
                ...
                val scopeIds =
                    documentRepository
                        .findScopeIds(command.documentId)
                        .mapLeft { error ->
                            when (error) {
                                is RepositoryReadError.NotFoundError -> ConfirmError.DocumentNotFoundError
                ...
                grantRepository
                    ...
                    .mapLeft { ConfirmError.GrantCreationError }
                    .bind()
            }
        }
}
