...
fun handleApiError(err: DomainError.ApiError): JsonApiErrorObject {
    when (err) {
        DomainError.ApiError.AuthorizationIdIsMalformed -> return JsonApiErrorObject(
            status = HttpStatusCode.BadRequest.value.toString(),
            title = "Malformed ID",
            detail = "The provided ID is not valid"
        )

        DomainError.ApiError.AuthorizationIdIsMissing -> return JsonApiErrorObject(
            status = HttpStatusCode.BadRequest.value.toString(),
            title = "Missing ID",
            detail = "Authorization ID is required"
        )

        is DomainError.ApiError.AuthorizationPayloadInvalid -> return JsonApiErrorObject(
            status = HttpStatusCode.BadRequest.value.toString(),
