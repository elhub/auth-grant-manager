...
import no.elhub.auth.v0.putPdf
...
class RouteTest : FunSpec({
    ...
    test("PUT /{id}.pdf returns 422 when handler fails to validate signature") {
    ...
    test("PUT /{id}.pdf returns 400 when id is invalid") {
        testApplication {
            setupAppWith(authorizedOrg) { route(handler) }
    ...
    test("PUT /{id}.pdf returns 400 when document is empty") {
        testApplication {
            setupAppWith(authorizedOrg) { route(handler) }
