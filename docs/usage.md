...
request authorization from a user to carry out an operation in Elhub on their behalf.

## Context

### Who is this for?

This API can be used by market actors who need to request authorization from end users for operations in Elhub. The physical
...
The Authorization Grant Manager API provides two primary interaction flows for requesting authorizations:
AuthorizationRequest and AuthorizationDocument. Both flows can be used to request authorization from end users.

The AuthorizationRequest flow is used to initiate and manage an authorization request through the Elhub
...
The AuthorizationDocument flow is used to request authorization from end users without the need for a redirect to Elhub.
...
Party can request authorization from end users without the need for a redirect to the Elhub MyPage. The solution will
provide a digitally signed document for Elhub to verify that the user has authorized the operation as described
by the document.
...
* All API requests to Elhub MUST include these headers:
    ```text
    Authorization: Bearer {token}
    SenderGln: {GLN}
    ```
  The `Authorization` header carries the Maskinporten access token. The `SenderGln` header defines the GLN the caller operates as for the request.
...
3. **Document Generation by Elhub**<br>
    * Elhub generates an authorization document (PDF) and returns a document ID.
